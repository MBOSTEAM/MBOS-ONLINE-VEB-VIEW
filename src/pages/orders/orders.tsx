...
export default function OrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    switch (tab) {
      ...
      case "yesterday":
        return orders.filter(
          (order) => formatDate(order.scheduled_datetime) === "yesterday"
        );
      case "today":
      default:
        return orders.filter(
          (order) => formatDate(order.scheduled_datetime) === "today"
        );
    }
  }, [tab, ordersData]);
    ...
    <div className="px-4 pt-6 max-w-xl mx-auto">
      <header className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <Button variant="ghost" size="icon-lg" aria-label="filter">
            <Menu size={30} />
          </Button>
        </div>
      </header>
        ...
        {filteredOrders.map((order) => {
          ...
          return (
            <Link to={`/orders/${order.id}`}>
              <Card
                key={order.id}
                className="flex flex-row items-stretch hover:shadow-md transition-shadow cursor-pointer"
              >
